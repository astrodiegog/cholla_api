...
class ChollaOnTheFlyAnalysisViz:
    ...
    def get_combofPath(self, nSkewerOutput, img_ext='png'):
        ...
        return self.ImgDir + "/" + f"OTFSkew_nOut{nSkewerOutput:.0f}_id{self.OTFSkew.OTFSkewerHead.skew_id:.0f}" + "." + img_ext
	...
	_ = ax_density.scatter(x_arr, np.log10(density_prerestart))
	_ = ax_HIdensity.scatter(x_arr, np.log10(HIdensity_prerestart))
	_ = ax_HeIIdensity.scatter(x_arr, np.log10(HeIIdensity_prerestart))
	_ = ax_vel.scatter(x_arr, vel_prerestart)
	_ = ax_temp.scatter(x_arr, np.log10(temp_prerestart))
	...
	# add skew id
	skew_label = r'$\rm{skewid}$' + rf'$- {skewid}$'
	skew_coords_y = 2.5
	skew_coords_x = self.OTFSkew.OTFSkewerHead.n_i * di
	if skewid < 1e1: # one digit
	    skew_coords_x *= 0.88
	elif skewid < 1e2: # two digit
	    skew_coords_x *= 0.86
	elif skewid < 1e3: # three digit
	    skew_coords_x *= 0.85
	elif skewid < 1e4: # four digit
	    skew_coords_x *= 0.83
	elif skewid < 1e5: # five digit
	    skew_coords_x *= 0.81
