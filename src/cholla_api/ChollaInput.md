...
Within the global $j-k$ plane, batches of $N_{\rm{box, skewers}}$ are tiled first along the $k$-dimension and $j$-dimension. For some global skewer with the global id $\eta_{\rm{global}}$, the box in which it is found is $\eta_{\rm{box}} = \rm{int} (\eta_{\rm{global}} // N_{\rm{box, skewers}} )$. In effect, the _global_ offsets for some box along the $j$-dimension is $j_{\rm{box, offset}} = \rm{int}( \eta_{\rm{global}} \rm{mod} ( n_{\rm{proc},j} ) \cdot n_{\rm{box},j} )$. Likewise, the offset along the $k$-dimension is $k_{\rm{box, offset}} = \rm{int}( \eta_{\rm{global}} // ( n_{\rm{proc},j} ) \cdot n_{\rm{box},k}) )$. These values are computed in the function ``AnalysisModule::Transfer_Skewers_Data(int axis)`` on line 1037 of ``src/analysis/lya_statistics.cpp``.
